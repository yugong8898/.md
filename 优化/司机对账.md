...
graph TB
    ...
    D --> E[getDetail 获取详情]
    E --> F[getDetail 返回处理]
    F --> F1[按 transportType + settlementType<br/>动态替换 formList 9~11 项]
    F --> F2[处理 bmsBusinessCostList<br/>补空小计行 + 格式化金额]
    F --> F3[fetchOilGasConfig 获取油气返利配置]
    F3 --> F4[$total 计算特殊汇总行]
    F --> F5[getRecord 加载操作记录<br/>createDate >= 2024-02-01]
```
