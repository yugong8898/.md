...
**依赖链**：`$total` 同时依赖 `this.reseponse`（费用数据）和 `this.oilGasConfig`（油气配置），所以必须在两者都就绪后执行：

> getDetail 返回 → this.reseponse = res（赋值）→ fetchOilGasConfig()（读 reseponse 拿 networkMainBodyId）→ $total()（读费用列表 + oilGasConfig）

> `fetchOilGasConfig` 失败时走 catch 降级执行 `$total`，此时 `oilGasConfig.hasConfig = false`，不展示油气优惠行。

### 12.3 fetchOilGasConfig 油气配置获取

`fetchOilGasConfig` 调用 `queryDispatchRatio` 接口，获取网货主体 + 企业维度的油气返利配置，结果存入 `this.oilGasConfig`，供 `$total` 判断是否展示油气优惠行。

```mermaid
graph LR
    A["fetchOilGasConfig"] --> B{有 networkMainBodyId<br/>+ companyId?}
    B -->|否| C["oilGasConfig.hasConfig = false"]
    B -->|是| D["调 queryDispatchRatio 接口"]
    D --> E{response.model 有数据?<br/>model的第0个}
    E -->|是| F["hasConfig = result.hasConfig<br/>configSource = COMPANY 或 DEFAULT<br/>canDisplay = hasConfig 且来源合法"]
    E -->|否| C
    F --> G["oilGasConfig = { hasConfig, configSource }"]
```

### 12.4 确认金额编辑逻辑（verify 模式）
...
1. **弹出末尾空对象**：`bmsBusinessCostList.pop()`，移除 `getDetail` 时补的空行
2. **reduce 累加**：遍历所有费用行，`costType` 为 `200`/`600`（应付/其他应付）做加法，其余做减法
3. **格式化小计金额**：`parsePrice(parsePriceReduction())` 统一格式
4. **追加特殊行**：按条件依次追加 subtotal → freightDiff → insurance → oilDiscount → total → fpDiff/shipperPay/carrierPay

累加计算核心：

```javascript
const isAdd = ['200', '600'].includes(`${current.costType}`) // 应付加 / 其他应付加
// 小计 = 加项 - 减项
// 调整金额 = 对应的 updBusCost 同样加减
// busCost = adjustCost - costName（调整后差额）
```
