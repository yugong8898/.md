...
`created` 阶段还会处理 URL 中的 `waybillId`、默认网络货运主体和近 3 个月的默认装货时间。
...
graph TB
    ...
    B -->|否| D[校验运单状态]
    D --> E{存在异常运单?}
    E -->|是| F[warnTable 展示异常]
    E -->|否| G[批量风控测算]
    G --> H{存在中高风险?}
    H -->|是| I[RiskWarningDialog 确认]
    H -->|否| J[提交结算]
    I -->|继续| J
    I -->|取消| K[停止提交]
    J --> L[汇总成功/失败原因]
...
graph LR
    A[选择结算处理] --> B{处理类型}
    B -->|终止| C[二次确认]
    C --> D[settlementFlag=2]
    B -->|转移| E[选择目标网络货运主体]
    E --> F[二次确认]
    F --> G[settlementFlag=1 + 主体/税率]
    D --> H[调用 waybillStopSettlement]
    G --> H
    H --> I[刷新列表]
```
...
扩展字段包括手机号、开户行、单价、税率、服务费和货物保障服务金额等。
...
```mermaid
graph LR
    A[修改单价/结算数量] --> B{允许自动重算?}
    B -->|否| C[保持原运费]
    B -->|是| D[单价 × 结算数量]
    D --> E[按 roundingMode 取整]
    E --> F[同步费用明细运费项]
```

`updBusCostInput()` 后开启自动重算；`POW` 开头且 `supplyType='300'` 的运单不自动重算。
...
graph TB
    ...
    B --> C[校验油气费]
    C --> D{是否需要抹零?}
    D -->|应付差额| E[追加应付抹零费项 101]
    D -->|应收差额| F[追加应收抹零费项 102]
    D -->|无需抹零| G[校验表单]
    E --> G
    F --> G
    G --> H[校验运单状态]
...
> 付款等式校验（小计 = 预付 + 回单付 + 到付）和回单付比例校验由 `SettlementInfo.vue` 内部处理。

### 8.7 风控处理说明

保存和提交共用上图中的风控分支：先保存当前表单数据，再调用 `getWaybillRiskInfoList()` 测算；存在中高风险时由 `RiskWarningDialog` 决定继续或停止。

| 回调 | 作用 |
|------|------|
| `handleCloseRiskDialog` | 关闭弹窗并停止操作 |
| `handleSubmitRiskDialog` | 确认后继续保存/提交 |
