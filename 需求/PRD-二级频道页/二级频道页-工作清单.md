...
### 3.1 数据结构（`types.ts`，字段名占位）

```ts
interface AgentMarketModule {
  enabled: boolean;          // 模块启用/禁用
  displayTitle: string;      // 前台展示名称
  agents: AgentItem[];       // 智能体列表，≤6
  showMore: boolean;         // 查看更多开关
  showMoreLink: string;      // 查看更多链接
}

interface AgentItem {
  id?: string;               // 编辑用临时 id（前端生成）
  name: string;              // 智能体名称，必填
  intro: string;             // 简介，选填
  coverImage: string;        // PC 端封面图，选填
  link: string;              // 跳转链接，选填
  sort: number;              // 展示顺序
...
- 启用/禁用切换即时生效（改列表数据），禁用项**配置保留**、前台不展示；
- 删除：`ElMessageBox.confirm`「确认删除该智能体？删除后前台不再展示。」，确认后移除；
- 排序：拖拽行或顺序值（待确认 #5），排序结果写入 `sort`。
...
| 5 | `link` 有值时必须格式合法 | 「智能体跳转链接格式不正确」 |
| 6 | 开启查看更多时 `showMoreLink` 必填且格式合法 | 「请填写查看更多链接 / 链接格式不正确」 |

### 3.6 `submit.ts` payload 草案

```ts
{
  moduleKey: 'agent-market',
  data: {
    enabled, displayTitle,
    agents: agents.map(a => ({ name, intro, coverImage, link, sort, enabled })),
...
- 智能体卡片列表（按展示顺序）：封面图（预览/缺省态）、名称、简介、跳转链接（可点击、新窗口）；操作列隐藏；
- 查看更多：开启时展示可点击链接；
...
- [x] 智能体列表 + 启用禁用/编辑/删除
- [x] 排序实现（原生 HTML5 拖拽，待确认 #5）
- [x] `validate.ts`（3.5 六条）
- [x] `submit.ts`（3.6 结构，字段名待确认 #11）
- [x] `index.vue` readonly 只读模式（3.7 内容）
- [ ] 产品确认：名称/简介长度上限（待确认 #12）
- [ ] 后端确认：字段名映射（待确认 #11）
...
- 删除：确认弹窗「确认移除该活动？移除后不影响已选服务产品。」——**删除活动不影响已选服务产品**。

### 4.6 服务产品选择弹窗（独立子组件，如 `ServiceSelectDialog.vue`）

- **无需先选择活动**，直接从当前活动中的全部服务产品选择；
...
**文档版本**: v4.3（同步开发进度与阻塞项到工作清单，便于团队追踪）  
**历史版本**: v4.2（统一上传组件约定修正）  
**最后更新**: 2026年09月  
**整理人**: 王新骏
